...
float sending_timer;
bool complianceMode = 0;


//PID parameters - tuned by the user
float proportional = -100; //1.35; //k_pp
float integral = 0.005;//0.00005; //k_i 
float derivative = 0.5; //0.01 k_d 
...
// should wait to receive current angle from joint 2
...
//  checkQuadrant(2);
  ...
  globalAngles[1] = preloaded_trajectory[i][1]*57.2958; // needs to be sent to joint2 

//  canSender(globalAngles[1]); 
...
//  if(!complianceMode){
//    canReceiver(); // need current angle of joint 2, state of whether it has slipped or not. [ int joint angle, decimal component of joint angle, slip state]
//  }
//  
...
void move_motors(float angles[2])
{
  ...
  float j1CurrPosition = totalAngle[0]; // angle of encoder 1 of joint 1
...
//------------------------------------------------------------------------------------//
/*
 * Functions for communication with other joints.
 * 
 */
...
void canSender(float jointAngle) {
  // send packet: id is 11 bits, packet can contain up to 8 bytes of data
//  Serial.print ("Sending joint 2's angle ... ");
  CAN.beginPacket (0x12); //sets the ID and clears the transmit buffer
  // CAN.beginExtendedPacket(0xabcdef);

  int integerAngleComponent = (int) jointAngle;
  int decimalAngleComponent = (int) 10000*(jointAngle - integerAngleComponent); // last 4 decimals (may approximate the 4th digit but accuracy of that degree not important).
  
  CAN.write (integerAngleComponent); //write data to buffer. data is not sent until endPacket() is called.
  CAN.write (decimalAngleComponent);
  CAN.endPacket();
