...
int lowbyte; //raw angle 7:0 registers
word highbyte; //raw angle 7:0 and 11:8
int rawAngle; //final raw angle , combines the lowbyte and highbyte registers
float degAngle; //raw angle in degrees (360/4096 * [value between 0-4095])

int quadrantNumber, previousquadrantNumber; //quadrant IDs, we will be tracking which quadrant the encoder is in.
float numberofTurns = 0; //number of turns
float correctedAngle = 0; //tared angle - based on the startup value
float startAngle = 0; //starting angle  , for aligning the value of the encoder's angle
float totalAngle = 0; //total absolute angular displacement absolute angle irrespective of the quadrant we're in.
...
{  
  ...
  while((magnetStatus & 32) != 32) //while the magnet is not adjusted to the proper distance - 32: MD = 1  (0xb100000)
  {
