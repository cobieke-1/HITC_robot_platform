...
class PlatformStatePublisherNode(Node):
    def __init__(self):
        ...
        self.subscriber_ = self.create_subscription(Float32MultiArray,'/micro_ros_arduino_node_publisher', self.send_angles)
    ...
    def timer_callback(self): #should send jointangles to esp32 (currently still sends directly to robot simulation)
        # Update joint state
    ...
    def send_angles(self, msg): # after recieving the encoder's angles then send these angles to the robot simulation
        now = self.get_clock().now()
        self.joints.header.stamp = now.to_msg()
        self.joints.name = ['base_L1', 'L1-L2','L2-EE'] 

        self.joints.position = msg.data
        if (self.count < len(self.results)-1):
            self.count += 1
        self.publisher_.publish(self.joints)
        self.publisher_micro_ros.publish(motor_joints)
